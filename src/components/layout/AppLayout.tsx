      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-4 md:px-6">
          <div className="flex items-center gap-3">
            ...
            {activeChannel && (callPresence.get(activeChannel.id)?.length ?? 0) > 0 && (
              <span className="ml-1" title="In call">
                <CallAvatars participants={callPresence.get(activeChannel.id)!} max={5} />
              </span>
            )}
          </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-xl px-4 py-2 text-sm transition-colors ${
                  callActive && activeChannel.id === callChannelId
                    ...
                    : (callPresence.get(activeChannel.id)?.some((p) => p.userId !== userId)
              ...
              >
                {callActive && activeChannel.id === callChannelId ? (
                  ...
                  <>
                    ...
                    {callPresence.get(activeChannel.id)?.some((p) => p.userId !== userId) ? "Join call" : "Call"}
                  </>
                )}
              </button>
